...
export default function Page() {
    const { register, formState: { errors }, setValue } = useForm<Devotiee_interface>({ mode : "onBlur"});
    const userData = useSelector((state: any) => state.devotee.editUserData);
    
    useEffect(() => {
        if (userData){
            for (const iter in userData){
                if (iter === "InitiationDate" && userData.InitiationDate){
                    setValue(iter,new Date(userData.InitiationDate).toISOString().split("T")[0]);
                }else{
                    setValue(iter,userData[iter]);
                }
            }
        }
    },[])
            ...
            <div className="container mt-5 p-4 w-50 shadow card">
                <form action={handleSubmition} encType="multipart/form-data">
                    <div className="row">
                        ...
                        <input hidden {...register("DevoteeId")}/>
                    </div>
                    <input type="text" hidden {...register("_id")} />
